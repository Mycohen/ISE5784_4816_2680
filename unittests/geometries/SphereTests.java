...
class SphereTests {
    ...
    void testFindIntersections() {
        ...
        assertNull(sphere.findIntersections(rayTanOn), "Ray is tangent to the sphere, starts on the sphere");

        // TC04: Ray starts inside the sphere (1 point)
        Ray rayIn = new Ray(new Point(0.5, 0.5, 0.5), new Vector(1, 0, 0));
        var resIn = sphere.findIntersections(rayIn);
        assertEquals(1, resIn.size(), "Method did not find 1 intersection point");

        // TC05: Ray starts before and crosses the sphere (2 points)
        var resCross = sphere.findIntersections(new Ray(p100, new Vector(-1.665278089022151, -0.12235841760011, 1.736500836326363)));
        ...
        var resInOut = sphere.findIntersections(rayInOut);
        ...
        var resOnNotCenter = sphere.findIntersections(rayOnNotCenter);
        ...
        var resOnNotCenterRev = sphere.findIntersections(rayOnNotCenterRev);
        ...
        assertNull(sphere.findIntersections(rayTanAfter), "Method did not find 0 intersection points Ray is tangent to the sphere, starts after the sphere");

        // TC11: Ray is right angle to the center (0 points)
        Ray rayRightAngle = new Ray(new Point(0, 0, 3), new Vector(0, 1, 0));
        assertNull(sphere.findIntersections(rayRightAngle), "Method did not find 0 intersection points Ray is right angle to the center");
        ...
        var resProjCenter = sphere.findIntersections(rayProjCenter);
        ...
        var resOutCrossCenter = sphere.findIntersections(rayOutCrossCenter);
        ...
        var resOnCenter = sphere.findIntersections(rayOnCenter);
        ...
        assertNull(sphere.findIntersections(rayProjNoIntersect), "Method did not find 0 intersection points when the Ray projection intersects the center and the ray does not intersect the sphere");
        ...
        assertNull(sphere.findIntersections(rayOnProjCenter), "Method did not find 0 intersection points when the Ray starts on the sphere and the projection intersects the sphere through its center");
    }
